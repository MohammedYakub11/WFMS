  ...
  async searchWorkforce(query: SearchQueryDto) {
    ...
    const queryBuilder = this.employeeRepository
      ...
      .leftJoinAndSelect(
        ...
        'employeeSkill',
        'employeeSkill.approvalStatus = :status',
        { status: 'approved' },
      )
    ...
    if (
      ...
      queryBuilder.andWhere(
        new Brackets((qb) => {
          const subQuery = this.employeeSkillRepository
          ...
          if (proficiency) {
            const profNum =
              typeof proficiency === 'string'
                ? parseInt(proficiency, 10)
                : proficiency;
            subQuery.andWhere('es.proficiencyRating >= :proficiency', {
              proficiency: profNum,
            });
          }
          if (certified) {
            subQuery.andWhere('es.isCertified = :certified', {
              certified: true,
            });
          }
          if (experienceMin) {
            const expMinNum =
              typeof experienceMin === 'string'
                ? parseInt(experienceMin, 10)
                : experienceMin;
            subQuery.andWhere('es.yearsOfExperience >= :expMin', {
              expMin: expMinNum,
            });
          }
          if (experienceMax) {
            const expMaxNum =
              typeof experienceMax === 'string'
                ? parseInt(experienceMax, 10)
                : experienceMax;
            subQuery.andWhere('es.yearsOfExperience <= :expMax', {
              expMax: expMaxNum,
            });
          }

          qb.where(
            'employee.id IN (' + subQuery.getQuery() + ')',
    ...
    const items = employees.map((emp) => {
      ...
      const primarySkills = approvedSkills
        .slice(0, 3)
        .map((es) => es.skill.skillName);

      const avgProficiency =
        approvedSkills.length > 0
          ? approvedSkills.reduce(
              (acc, es) => acc + (es.proficiencyRating || 0),
              0,
            ) / approvedSkills.length
          : 0;

      const certifications = approvedSkills.filter(
        (es) => es.isCertified,
      ).length;
