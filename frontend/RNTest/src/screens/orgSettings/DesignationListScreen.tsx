...
import { useDeleteDesignation, useDesignations } from '../../hooks/useDesignations';
...
import { Designation } from '../../types/organization';

const LIMIT = 10;
type StatusFilter = 'active' | 'inactive' | null;

export const DesignationListScreen = () => {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useDesignations(query, page, LIMIT);
  const deleteMutation = useDeleteDesignation();

  const items: Designation[] = data?.items || [];
  ...
  const handleDelete = () => {
    ...
    deleteMutation.mutate(toDelete.id, {
      onSuccess: () => {
        showSnackbar('Designation deleted', 'success');
        setToDelete(null);
      },
      onError: () => {
  ...
  return (
    <View style={styles.container}>
      ...
      {isError ? (
        <EmptyState title="Failed to load designations" description="Please try again." actionTitle="Retry" onAction={handleRefresh} />
      ) : (
        <FlatList
          data={items}
          ...
          renderItem={({ item }) => (
            <View style={styles.cardWrapper}>
              <OrgEntityCard
                ...
                canManage={canManage}
                onPress={() => navigation.navigate('DesignationForm', { designationId: item.id })}
          ...
          ListHeaderComponent={
            <View style={styles.headerContent}>
              <AppTextField label="" placeholder="Search designations..." value={localSearch} onChangeText={setLocalSearch} style={styles.searchField} />
              {renderStatusToggle()}
            </View>
          }
          ...
          ListEmptyComponent={!isLoading ? <EmptyState title="No designations found" description="Create your first designation to get started." /> : null}
      ...
      {canManage && <FAB icon={renderAppIcon("plus")} style={styles.fab} onPress={() => navigation.navigate('DesignationForm', {})} color="#FFF" />}
